...
public class Dao {
	...
	protected static String dbUrl = "jdbc:sqlserver://localhost:1433;DatabaseName=libraryManager";
	...
	private Dao() {
		try {
			if (conn == null) {
				...
				System.out.println("你好飞哥，数据库连接成功！");
			}
			else
				return;
		} catch (Exception ee) {
	 ...
	 * 
	 */
	public static int InsertBookType(String bookTypeName,String days,String fk,String id){
		int i=0;
		try{
			String sql="insert into tb_bookType(typeName,days,fk,id) values('"+bookTypeName+"','"+days+"','"+fk+"','"+id+"')";
	 ...
	 * 修改图书信息方法
	 */
	public static int Updatebook(String ISBN,String typeId,String bookname,String writer,String translator,String publisher,Date date,String price){
		int i=0;
		try{
 ...
 */
	...
	public static int Insertoperator(String name,String sex,int age,String identityCard,Date date,String tel,String password,String id){
		int i=0;
		try{
			String sql="insert into tb_operator(name,sex,age,identityCard,workdate,tel,password,id) values('"+name+"','"+sex+"','"+age+"',"+identityCard+",'"+date+"','"+tel+"','"+password+"','"+id+"')";
			System.out.println(sql);
			i=Dao.executeUpdate(sql);
		}catch(Exception e){
			e.printStackTrace();
